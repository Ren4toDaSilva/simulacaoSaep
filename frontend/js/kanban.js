...
function renderKanban() {
  ["a_fazer", "fazendo", "pronto"].forEach((status) => {
    ...
    if (!items.length) {
      col.innerHTML = '<div class="empty-col">Nenhuma tarefa</div>';
      return;
    }
    ...
    col.innerHTML = items
      ...
      <div class="task-card" id="card-${t.id}">
        <div class="d-flex justify-content-between align-items-start">
          <span class="badge-prioridade badge-${t.prioridade}">${
        ...
        <div class="task-desc">${escHtml(t.descricao)}</div>
        <div class="task-meta mb-2">
          <i class="bi bi-building"></i> ${escHtml(t.setor)}
          &nbsp;·&nbsp;
          <i class="bi bi-person"></i> ${escHtml(t.nome_usuario)}
        </div>
        <div class="d-flex justify-content-between align-items-center task-actions flex-wrap gap-1">
          <select class="status-select" onchange="alterarStatus(${
            t.id
          }, this.value)">
          ...
          <div class="d-flex gap-1">
            <button class="btn btn-outline-primary btn-icon" onclick="abrirEdicao(${
              t.id
            })">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-outline-danger btn-icon" onclick="confirmarExclusao(${
              t.id
            })">
